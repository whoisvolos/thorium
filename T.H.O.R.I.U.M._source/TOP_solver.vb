...
Public Class TOP_solver
    ...
    ''' <summary>
    ''' Argument for inside of Solver calculations.
    ...
    ''' <summary>
    ''' Temperatures (in K) of elements during the solution process
    ''' </summary>
    ''' <remarks></remarks>
    Public T(,) As Double

    ''' <summary>
    ''' Heat load (in W) of elements during the solution process
    ''' </summary>
    ''' <remarks></remarks>
    Public Q(,) As Double
    ...
    ''' <summary>
    ''' Color codes for all of element faces at all time steps
    ''' </summary>
    ''' <remarks></remarks>
    Public Color_codes(,) As Face_code


    ''' <summary>
    ''' One can obtain outcome radiation power from elements by scalar multiplication of Q_R_outcome_vector and vector of temperatures on given time step
    ''' </summary>
    ''' <remarks></remarks>
    Public Q_R_outcome_vector() As Double
    ...
    Public Sub New(ByRef FEM As Model)
        ...
        Me.Save_step_time = FEM.Save_step_time
    ...
    Public Sub Fill_HT_element()
        ...
        For i As Integer = 0 To UBound(HT_element)
            ...
            For j As Integer = 0 To HT_element(i).Element.N_Neighbor
                ...
                HT_element(i).Heat_conductivity(j).Area_1 = HT_element(i).Element.Neighbor(j).Area_1

                HT_element(i).Heat_conductivity(j).Area_2 = HT_element(i).Element.Neighbor(j).Area_2
    ...
    Public Sub Init_temperature_field()
        ...
        If Not (FEM.Is_thermooptics_constant) Then
        ...
        For i As Integer = 0 To UBound(HT_element)
            ...
            HT_element(i).T_0 = HT_element(i).Element.Field.Temperature
    ...
    Public Sub Fill_B_matrices()
        ...
        For i As Integer = 0 To UBound(HT_element)
            ...
            If Progress + 1 <= i / UBound(HT_element) * 100 Then
                ...
                If 1 / UBound(HT_element) * 100 > 1 Then

                    Progress += 1 / UBound(HT_element) * 100
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub Fill_B_matrices_TEST()
        ...
        ' Вот это и есть нижеописанная строчка матрицы B, записанная для граней
        Dim B_row_faces(UBound(FEM.Shining_face)) As Double

        ' Вот это и есть нижеописанная строчка матрицы B, записанная для элементов
        Dim B_row_HT_elements(UBound(HT_element)) As Double


        Dim N_elem_emitter, N_elem_absorber As Integer
        ...
        ' Инициализируем массивы лучистых связей

        Dim HT_element_i__Heat_radiation(UBound(HT_element)) As Integer

        For i As Integer = 0 To UBound(HT_element)

            ReDim HT_element(i).Heat_radiation(UBound(HT_element))

            HT_element_i__Heat_radiation(i) = -1
        ...
        For i As Integer = 0 To UBound(FEM.Shining_face)
            ...
            ReDim B_row_HT_elements(UBound(HT_element))

            N_elem_emitter = FEM.Shining_face(i).HT_element.Number

            With FEM.Shining_face(i)

                B_row_faces = Illuminate_model(FEM.Shining_face(i), .oz, Math.PI / 2, .FTOP.CDF, .Argument, .Global_medium, aux_Visual, N_photon)

            End With

            For j As Integer = 0 To UBound(FEM.Shining_face)

                If B_row_faces(j) > 0 Then

                    N_elem_absorber = FEM.Shining_face(j).HT_element.Number

                    B_row_HT_elements(N_elem_absorber) += B_row_faces(j) * FEM.Shining_face(i).Area * FEM.Shining_face(i).FTOP.Eps * TOP_item.sigma
            ...
            For j As Integer = 0 To UBound(HT_element)
                ...
                If B_row_HT_elements(j) > 0 Then

                    HT_element_i__Heat_radiation(j) += 1

                    HT_element(j).Heat_radiation(HT_element_i__Heat_radiation(j)) = New Heat_radiation

                    HT_element(j).Heat_radiation(HT_element_i__Heat_radiation(j)).n_Emitter_HT_Element = N_elem_emitter

                    HT_element(j).Heat_radiation(HT_element_i__Heat_radiation(j)).Coefficient = B_row_HT_elements(j)

                End If

            Next j

            If Progress + 1 <= i / UBound(FEM.Shining_face) * 100 Then

                If 1 / UBound(FEM.Shining_face) * 100 > 1 Then

                    Progress += 1 / UBound(FEM.Shining_face) * 100
        ...
        For i As Integer = 0 To UBound(HT_element)
            ...
            If HT_element(i).Heat_radiation(0) Is Nothing Then

                HT_element(i).Heat_radiation(0) = New Heat_radiation

                HT_element(i).Heat_radiation(0).n_Emitter_HT_Element = 0

                HT_element(i).Heat_radiation(0).Coefficient = 0

            Else

                ReDim Preserve HT_element(i).Heat_radiation(HT_element_i__Heat_radiation(i))
    ...
    Public Sub Fill_Q_R_outcome_vector()

        ReDim Q_R_outcome_vector(UBound(HT_element))

        ' Рассчитываем тепловой поток во все стороны элемента за счет излучения

        For i As Integer = 0 To UBound(HT_element)

            Q_R_outcome_vector(i) = 0

            For j As Integer = 0 To UBound(HT_element(i).Face)

                Q_R_outcome_vector(i) += HT_element(i).Face(j).Area * HT_element(i).Face(j).FTOP.eps * TOP_item.sigma
    ...
    Public Sub Fill_conductivity_links()

        Dim Cond_1, Cond_2 As Double

        For i As Integer = 0 To UBound(HT_element)

            For j As Integer = 0 To UBound(HT_element(i).Heat_conductivity)

                Cond_1 = HT_element(i).Element.Medium.lambda(0) * HT_element(i).Heat_conductivity(j).Area_1 / HT_element(i).Heat_conductivity(j).Distance_1

                Cond_2 = HT_element(i).Heat_conductivity(j).HT_Element.Element.Medium.lambda(0) * HT_element(i).Heat_conductivity(j).Area_2 / HT_element(i).Heat_conductivity(j).Distance_2

                HT_element(i).Heat_conductivity(j).Conductivity = 1 / (1 / Cond_1 + 1 / Cond_2)
    ...
    Public Sub Calc_R_transfer_N_photon_old(ByRef Step_Number As Integer, ByRef N_photon As Integer, ByRef vf As Visualisation.Visualisation)
        ...
        For i As Integer = 0 To UBound(FEM.Shining_face)
            ...
            With FEM.Shining_face(i)
                ...
                Q(HT_element_Number, Step_Number + 1) += -FEM.Shining_face(i).Q_R_outcome + FEM.Shining_face(i).Q_R_income
    ...
    Public Sub Calc_R_transfer_N_photon(ByRef Step_Number As Integer, ByRef N_photon As Integer, ByRef vf As Visualisation.Visualisation)
        ...
        For i As Integer = 0 To UBound(FEM.Shining_face)
            ...
            With FEM.Shining_face(i)
                ...
                Q(HT_element_Number, Step_Number + 1) += -.Q_R_outcome + (.Q_R_income - .Q_R_diffuse_reflected - .Q_R_mirror_reflected - .Q_R_transmitted)
    ...
    ''' <param name="vf"></param>
    ''' <remarks></remarks>
    Public Function Calc_R_transfer_R_heat_source(ByRef vf As Visualisation.Visualisation) As Double(,)
    ...
    '                    HT_element(i).HT_Step(Step_Number + 1).Q += (.Area_1 + .Area_2) / 2 * (lambda_1 * .Distance_1 / (.Distance_1 + .Distance_2) + lambda_2 * .Distance_2 / (.Distance_1 + .Distance_2)) * (Temperature_2 - Temperature_1) / (.Distance_1 + .Distance_2)
    ...
    Public Sub Illuminate_model_and_save_absorbed_power_in_faces(ByRef Source As TOP_item, ByRef Direction As Vector, ByRef Opening_angle As Double, ByRef CDF As DEPEND, ByRef Argument As Vector, ByRef Emitter_medium As MEDIUM1, ByRef vf As Visualisation.Visualisation, ByRef N_photon As Integer, ByRef Total_outcoming_heat_power As Double)
        ...
        Else
            ...
            If UCase(Source.Type) = "RSOURCE1" Then
                ...
                R_source.Current_position = R_source.Calc_position

                to_Center = FEM.Center - R_source.Current_position
        ...
        If Face Is Nothing Then
            ...
            Photons = Photon.Generate_N_random_photons(R_source.Current_position, N_photon, New_opening_angle, CDF, Argument, Emitter_medium, Correction_coef * Total_outcoming_heat_power)
        ...
        For i As Integer = 0 To N_photon - 1
            ...
            If Source_distance > Limit_distance Then
                ...
                Photon.Center = Photon.Center + (L_vector(FEM.Center - Photon.Center) - FEM.Maximum_length) * Photon.Direction
    ...
    ''' <summary>
    ''' This sub calculates illimination of model from given RSOURCE1 with photons.
    ''' The photon emission is directed by Direction within Opening_angle.
    ''' The sub stores power of absorbed photons  in model's faces in elements.    
    ''' </summary>
    ''' <param name="R_source">Radiation source to emit photons</param>
    ''' <param name="vf">Visualisation form, used for photon-face interaction detection</param>
    ''' <remarks></remarks>
    Public Sub Illuminate_model_from_RSOURCE1_and_save_absorbed_power_in_faces(ByRef R_source As RSOURCE1, ByRef vf As Visualisation.Visualisation)

        Dim Total_outcoming_heat_power, T_eff As Double

        With R_source

            .Argument.Coord(1) = Argument.Coord(1)

            ' If radiation source emits like a black body then
            If .CDF Is Nothing Then

                If .Q.GetType.ToString = Total_outcoming_heat_power.GetType.ToString Then

                    Total_outcoming_heat_power = .Q

                Else

                    Total_outcoming_heat_power = .Calc_Q(.Argument)
            ...
            Else
                ...
                Total_outcoming_heat_power = sigma * T_eff * T_eff * T_eff * T_eff

            End If

            R_source.Current_position = .Calc_position(.Argument)
        ...
        Dim Photons(R_source.N_photon - 1) As Photon

        Photons = Photon.Generate_N_random_photons(R_source.Current_position, R_source.N_photon, New_opening_angle, R_source.CDF, R_source.Argument, R_source.Emitter_Medium, Correction_coef * Total_outcoming_heat_power)

        ' Reseting the results of previous interactions

        Reset_fases_R_Q()

        For i As Integer = 0 To R_source.N_photon - 1
    ...
    ''' <summary>
    ''' This function calculates incoming heat (in W) for given element number n_Element on given step n_Step.
    ''' The assumptions are: 1. Constant thermophysics. 2. Constant thermooptics
    ''' 3. Variable boundary conditions 4. Fixed radiation heat sources.
    ''' </summary>
    ''' <param name="n_Element">Element to calculate incoming heat</param>
    ''' <param name="n_Step">Time step to calculate incoming heat</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function Calc_Q_TP_C_TO_C_BC_V_RS_C(ByRef n_Element, ByRef n_Step) As Double

        Dim Q_R_income, Q_R_outcome, Q_lambda, Q_Heat_source, Q_Radio_source As Double

        ' Рассчитываем тепловой поток во все стороны элемента за счет излучения

        Q_R_outcome = Q_R_outcome_vector(n_Element) * T(n_Element, n_Step) ^ 4

        ' Рассчитываем тепловой поток к элементу от других элементов за счет излучения

        For i As Integer = 0 To UBound(T, 1)

            Q_R_income += B_matrix_element(i, n_Element) * TOP_item.sigma * T(i, n_Step) ^ 4
        ...
        For i As Integer = 0 To UBound(HT_element(n_Element).Heat_conductivity)

            ' Расcчитываем тепловую мощность

            Q_lambda += HT_element(n_Element).Heat_conductivity(i).Conductivity * (T(HT_element(n_Element).Heat_conductivity(i).HT_Element.Number, n_Step) - T(n_Element, n_Step))
        ...
        If UBound(HT_element(n_Element).Heat_source) >= 0 Then

            For i As Integer = 0 To UBound(HT_element(n_Element).Heat_source)

                ' Тепловыделение в источнике может зависеть от температуры элемента, так что добавляем ее в аргумент
                Argument.Coord(0) = T(n_Element, n_Step)

                Q_Heat_source += HT_element(n_Element).Heat_source(i).Heat(Argument)
        ...
        For i As Integer = 0 To UBound(FEM.RSOURCE)
            ...
            Q_Radio_source += FEM.RSOURCE(i).Calc_Q(Argument) * FEM.RSOURCE(i).B_row(n_Element)

        Next i

        Return Q_R_income - Q_R_outcome + Q_lambda + Q_Heat_source + Q_Radio_source
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function Calc_Q_TP_C_TO_C_BC_V_RS_C_TEST(ByRef n_Element, ByRef n_Step) As Double
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function Calc_Q_thermophysics_const_thermooptics_const_boundary_conditions_const(ByRef n_Element, ByRef n_Step) As Double
        ...
        If UBound(HT_element(n_Element).Heat_source) >= 0 Then
            ...
            For i As Integer = 0 To UBound(HT_element(n_Element).Heat_source)
                ...
                Q_Heat_source += HT_element(n_Element).Heat_source(i).Heat
        ...
        For i As Integer = 0 To UBound(FEM.RSOURCE)
            ...
            Q_Radio_source += FEM.RSOURCE(i).Calc_Q * FEM.RSOURCE(i).B_row(n_Element)
    ...
    ''' <summary>
    ''' This function calculates incoming heat (in W) for given element number n_Element on given step n_Step.
    ''' !!! Raduation sources arguments SHOLULD BE calculated before calling this function !!!
    ''' </summary>
    ''' <param name="n_Element">Element to calculate incoming heat</param>
    ''' <param name="n_Step">Time step to calculate incoming heat</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function Calc_Q_thermophysics_const_thermooptics_const_boundary_conditions_NOT_const(ByRef n_Element, ByRef n_Step) As Double
        ...
        ' передаем источникам тепла текущее время через аргумент всего Solver
        ...
        '    Q_Heat_source += FEM.RSOURCE(i).Calc_Q(FEM.RSOURCE(i).Argument) * FEM.RSOURCE(i).B_row(n_Element)
    ...
    Public Sub Calc_Q_const(ByRef HT_element As HT_Element)
        ...
        For i As Integer = 0 To UBound(HT_element.Heat_conductivity)
            ...
            With HT_element.Heat_conductivity(i)
                ...
                Q_lambda += (.Area_1 + .Area_2) / 2 * (lambda_1 * .Distance_1 / (.Distance_1 + .Distance_2) + lambda_2 * .Distance_2 / (.Distance_1 + .Distance_2)) * (Temperature_2 - Temperature_1) / (.Distance_1 + .Distance_2)
    ...
    ''' <param name="HT_element"></param>
    ''' <remarks></remarks>
    Public Sub Calc_Q_Radiation_const_Conductivity_const(ByRef HT_element As HT_Element)
    ...
    Sub Solve_problem()
        ...
        Select Case FEM.Solver_type
            ...
            Case "EULER"
                ...
                If FEM.Is_thermophysics_constant And FEM.Is_thermooptics_constant And FEM.Is_boundary_conditions_constant And FEM.Is_radiation_heat_sources_are_fixed Then

                    Fill_B_matrices()

                    Fill_Q_R_outcome_vector()

                    Fill_conductivity_links()

                    Euler_const_time_step_TP_C_TO_C_BC_C_RS_C()

                    Exit Select

                End If

                If Not (FEM.Is_thermophysics_constant) And FEM.Is_thermooptics_constant And FEM.Is_boundary_conditions_constant And FEM.Is_radiation_heat_sources_are_fixed Then

                    Throw New Exception("The Euler integration method with constant step for variable thermophysical properties, constant thermo-optical properties, constant boundary conditions and fixed radiation heat sources is not implemented yet. Sorry!")

                    Exit Select

                End If

                If FEM.Is_thermophysics_constant And Not (FEM.Is_thermooptics_constant) And FEM.Is_boundary_conditions_constant And FEM.Is_radiation_heat_sources_are_fixed Then

                    Throw New Exception("The Euler integration method with constant step for constant thermophysical properties, variable thermo-optical properties, constant boundary conditions and fixed radiation heat sources is not implemented yet. Sorry!")

                    Exit Select

                End If

                If Not (FEM.Is_thermophysics_constant) And Not (FEM.Is_thermooptics_constant) And FEM.Is_boundary_conditions_constant And FEM.Is_radiation_heat_sources_are_fixed Then

                    Throw New Exception("The Euler integration method with constant step for variable thermophysical properties, variable thermo-optical properties, constant boundary conditions and fixed radiation heat sources is not implemented yet. Sorry!")

                    Exit Select

                End If

                If FEM.Is_thermophysics_constant And FEM.Is_thermooptics_constant And Not (FEM.Is_boundary_conditions_constant) And FEM.Is_radiation_heat_sources_are_fixed Then
                    ...
                    Euler_const_time_step_TP_C_TO_C_BC_V_RS_C()
                ...
                If Not (FEM.Is_thermophysics_constant) And FEM.Is_thermooptics_constant And Not (FEM.Is_boundary_conditions_constant) And FEM.Is_radiation_heat_sources_are_fixed Then

                    Throw New Exception("The Euler integration method with constant step for variable thermophysical properties, constant thermo-optical properties, variable boundary conditions and fixed radiation heat sources is not implemented yet. Sorry!")
                ...
                If FEM.Is_thermophysics_constant And Not (FEM.Is_thermooptics_constant) And Not (FEM.Is_boundary_conditions_constant) And FEM.Is_radiation_heat_sources_are_fixed Then

                    Throw New Exception("The Euler integration method with constant step for constant thermophysical properties, variable thermo-optical properties, variable boundary conditions and fixed radiation heat sources is not implemented yet. Sorry!")
                ...
                If Not (FEM.Is_thermophysics_constant) And Not (FEM.Is_thermooptics_constant) And Not (FEM.Is_boundary_conditions_constant) And FEM.Is_radiation_heat_sources_are_fixed Then

                    Throw New Exception("The Euler integration method with constant step for variable thermophysical properties, variable thermo-optical properties, variable boundary conditions and fixed radiation heat sources is not implemented yet. Sorry!")
                ...
                If FEM.Is_thermophysics_constant And FEM.Is_thermooptics_constant And FEM.Is_boundary_conditions_constant And Not (FEM.Is_radiation_heat_sources_are_fixed) Then

                    Throw New Exception("The Euler integration method with constant step for constant thermophysical properties, constant thermo-optical properties, constant boundary conditions and movable radiation heat sources is not implemented yet. Sorry!")

                    Exit Select

                End If

                If Not (FEM.Is_thermophysics_constant) And FEM.Is_thermooptics_constant And FEM.Is_boundary_conditions_constant And Not (FEM.Is_radiation_heat_sources_are_fixed) Then

                    Throw New Exception("The Euler integration method with variavle step for constant thermophysical properties, constant thermo-optical properties, constant boundary conditions and movable radiation heat sources is not implemented yet. Sorry!")

                    Exit Select

                End If

                If FEM.Is_thermophysics_constant And Not (FEM.Is_thermooptics_constant) And FEM.Is_boundary_conditions_constant And Not (FEM.Is_radiation_heat_sources_are_fixed) Then

                    Throw New Exception("The Euler integration method with constant step for constant thermophysical properties, variable thermo-optical properties, constant boundary conditions and movable radiation heat sources is not implemented yet. Sorry!")

                    Exit Select

                End If

                If Not (FEM.Is_thermophysics_constant) And Not (FEM.Is_thermooptics_constant) And FEM.Is_boundary_conditions_constant And Not (FEM.Is_radiation_heat_sources_are_fixed) Then

                    Throw New Exception("The Euler integration method with constant step for variable thermophysical properties, variable thermo-optical properties, constant boundary conditions and movable radiation heat sources is not implemented yet. Sorry!")

                    Exit Select

                End If

                If FEM.Is_thermophysics_constant And FEM.Is_thermooptics_constant And Not (FEM.Is_boundary_conditions_constant) And Not (FEM.Is_radiation_heat_sources_are_fixed) Then
                    ...
                    Euler_const_time_step_TP_C_TO_C_BC_V_RS_V()
                ...
                If Not (FEM.Is_thermophysics_constant) And FEM.Is_thermooptics_constant And Not (FEM.Is_boundary_conditions_constant) And Not (FEM.Is_radiation_heat_sources_are_fixed) Then

                    Throw New Exception("The Euler integration method with constant step for variable thermophysical properties, constant thermo-optical properties, variable boundary conditions and movable radiation heat sources is not implemented yet. Sorry!")
                ...
                If FEM.Is_thermophysics_constant And Not (FEM.Is_thermooptics_constant) And Not (FEM.Is_boundary_conditions_constant) And Not (FEM.Is_radiation_heat_sources_are_fixed) Then

                    Throw New Exception("The Euler integration method with constant step for constant thermophysical properties, variable thermo-optical properties, variable boundary conditions and movable radiation heat sources is not implemented yet. Sorry!")
                ...
                If Not (FEM.Is_thermophysics_constant) And Not (FEM.Is_thermooptics_constant) And Not (FEM.Is_boundary_conditions_constant) And Not (FEM.Is_radiation_heat_sources_are_fixed) Then

                    Throw New Exception("The Euler integration method with constant step for variable thermophysical properties, variable thermo-optical properties, variable boundary conditions and movable radiation heat sources is not implemented yet. Sorry!")
            ...
            Case "EULER_V"
                ...
                If FEM.Is_thermophysics_constant And FEM.Is_thermooptics_constant And FEM.Is_boundary_conditions_constant And FEM.Is_radiation_heat_sources_are_fixed Then
                    ...
                    'Euler_var_time_step_TP_C_TO_C_BC_C_RS_C()
                ...
                If FEM.Is_thermophysics_constant And FEM.Is_thermooptics_constant And Not (FEM.Is_boundary_conditions_constant) And FEM.Is_radiation_heat_sources_are_fixed Then
                    ...
                    Fill_B_matrices_TEST()

                    Fill_Q_R_outcome_vector()

                    Fill_conductivity_links()

                    Euler_var_time_step_TP_C_TO_C_BC_V_RS_C_TEST()
            ...
            Case "ADAMS_4"
                ...
                Throw New Exception("The Adams integration method  is not implemented yet. Sorry!")
    ...
    Sub Resume_problem()
        ...
        If Me.Resumption_start_time = Double.MaxValue Then
            ...
            For i As Integer = UBound(time) To 0 Step -1

                If T(0, i) > 0 Then

                    N_step_start = i

                    Me.Start_time = time(i)
        ...
        Else
            ...
            For i As Integer = 0 To UBound(time) - 1

                If time(i) > Resumption_start_time Then
        ...
        Select Case FEM.Solver_type
            ...
            Case "EULER"
                ...
                If FEM.Is_thermophysics_constant And FEM.Is_thermooptics_constant And FEM.Is_boundary_conditions_constant Then

                    Fill_B_matrices()

                    Fill_conductivity_links()

                    'Euler_const_time_step_thermophysics_const_thermooptics_const_boundary_conditions_const()

                End If

                If Not (FEM.Is_thermophysics_constant) And FEM.Is_thermooptics_constant And FEM.Is_boundary_conditions_constant Then

                    Throw New Exception("The Euler integration method with constant step for variable thermophysical properties, constant thermo-optical properties and constant boundary conditions is not implemented yet. Sorry!")

                    Exit Select

                End If

                If Not (FEM.Is_thermophysics_constant) And Not (FEM.Is_thermooptics_constant) And FEM.Is_boundary_conditions_constant Then

                    Throw New Exception("The Euler integration method with constant step for variable thermophysical properties, variable thermo-optical properties and constant boundary conditions is not implemented yet. Sorry!")

                    Exit Select

                End If

                If FEM.Is_thermophysics_constant And Not (FEM.Is_thermooptics_constant) And FEM.Is_boundary_conditions_constant Then

                    Throw New Exception("The Euler integration method with constant step for constant thermophysical properties, variable thermo-optical properties and constant boundary conditions is not implemented yet. Sorry!")
                ...
                If FEM.Is_thermophysics_constant And FEM.Is_thermooptics_constant And Not (FEM.Is_boundary_conditions_constant) Then

                    Fill_B_matrices()

                    Fill_conductivity_links()

                    'Euler_const_time_step_thermophysics_const_thermooptics_const_boundary_conditions_NOT_const()

                End If

                If Not (FEM.Is_thermophysics_constant) And FEM.Is_thermooptics_constant And Not (FEM.Is_boundary_conditions_constant) Then

                    Throw New Exception("The Euler integration method with constant step for variable thermophysical properties, constant thermo-optical properties and variable boundary conditions is not implemented yet. Sorry!")

                    Exit Select

                End If

                If Not (FEM.Is_thermophysics_constant) And Not (FEM.Is_thermooptics_constant) And Not (FEM.Is_boundary_conditions_constant) Then

                    Throw New Exception("The Euler integration method with constant step for variable thermophysical properties, variable thermo-optical properties and variable boundary conditions is not implemented yet. Sorry!")

                    Exit Select

                End If

                If FEM.Is_thermophysics_constant And Not (FEM.Is_thermooptics_constant) And Not (FEM.Is_boundary_conditions_constant) Then

                    Throw New Exception("The Euler integration method with constant step for constant thermophysical properties, variable thermo-optical properties and variable boundary conditions is not implemented yet. Sorry!")
            ...
            Case "EULER_V"
                ...
                If FEM.Is_thermophysics_constant And FEM.Is_thermooptics_constant And FEM.Is_boundary_conditions_constant Then
                    ...
                    'Euler_var_time_step_thermophysics_const_thermooptics_const_boundary_conditions_const()

                End If

                If Not (FEM.Is_thermophysics_constant) And FEM.Is_thermooptics_constant And FEM.Is_boundary_conditions_constant Then

                    Throw New Exception("The Euler integration method with variable step for variable thermophysical properties, constant thermo-optical properties and constant boundary conditions is not implemented yet. Sorry!")
                ...
                If Not (FEM.Is_thermophysics_constant) And Not (FEM.Is_thermooptics_constant) And FEM.Is_boundary_conditions_constant Then
                    ...
                    Throw New Exception("The Euler integration method with variable step for variable thermophysical properties, variable thermo-optical properties and constant boundary conditions is not implemented yet. Sorry!")
                ...
                If FEM.Is_thermophysics_constant And Not (FEM.Is_thermooptics_constant) And FEM.Is_boundary_conditions_constant Then
                    ...
                    Throw New Exception("The Euler integration method with variable step for constant thermophysical properties, variable thermo-optical properties and constant boundary conditions is not implemented yet. Sorry!")
                ...
                If FEM.Is_thermophysics_constant And FEM.Is_thermooptics_constant And Not (FEM.Is_boundary_conditions_constant) Then
                    ...
                    'Euler_var_time_step_thermophysics_const_thermooptics_const_boundary_conditions_NOT_const()

                End If

                If Not (FEM.Is_thermophysics_constant) And FEM.Is_thermooptics_constant And Not (FEM.Is_boundary_conditions_constant) Then

                    Throw New Exception("The Euler integration method with variable step for variable thermophysical properties, constant thermo-optical properties and variable boundary conditions is not implemented yet. Sorry!")
                ...
                If Not (FEM.Is_thermophysics_constant) And Not (FEM.Is_thermooptics_constant) And Not (FEM.Is_boundary_conditions_constant) Then
                    ...
                    Throw New Exception("The Euler integration method with variable step for variable thermophysical properties, variable thermo-optical properties and variable boundary conditions is not implemented yet. Sorry!")
    ...
    ''' <summary>
    ''' This sub solve problem using simple Euler integration method. The assumptions are: 1. Constant time step. 2. Constant thermophysics. 3. Constant thermooptics 4. Variable boundary conditions 5. Movable radiation heat sources.
    ''' </summary>
    ''' <remarks></remarks>
    Sub Euler_const_time_step_TP_C_TO_C_BC_V_RS_V()
        ...
        ReDim time(Lenght_time / Step_time)

        time(0) = Start_time

        Dim Progress As Integer = 0

        ' Инициализируем массивы для хранения данных во время расчета
        ReDim T(UBound(HT_element), UBound(time))

        ReDim Q(UBound(HT_element), UBound(time))


        ' Заполнение начальных условий
        For i As Integer = 0 To UBound(T, 1)

            T(i, 0) = HT_element(i).T_0
        ...
        RaiseEvent Solution_progress(0)

        For n As Integer = 1 To Lenght_time / Step_time

            time(n) = time(n - 1) + Step_time

            Argument.Coord(1) = time(n)
            ...
            For i As Integer = 0 To UBound(FEM.T_SET)
                ...
                For j As Integer = 0 To UBound(FEM.T_SET(i).Element)
                    ...
                    T(FEM.T_SET(i).Element(j).HT_element.Number, n) = FEM.T_SET(i).Temp(j)
            ...
            ' Power absorbed by faces and emitted from radiation heat source calculation

            For i As Integer = 0 To UBound(FEM.RSOURCE)

                Illuminate_model_from_RSOURCE1_and_save_absorbed_power_in_faces(FEM.RSOURCE(i), aux_Visual)

            Next i

            For i As Integer = 0 To UBound(FEM.Shining_face)

                Q(FEM.Shining_face(i).HT_element.Number, n - 1) += FEM.Shining_face(i).Q_R_absorbed
            ...
            For i As Integer = 0 To UBound(T, 1)
                ...
                If HT_element(i).T_SET Is Nothing Then
                    ...
                    Q(i, n - 1) += Calc_Q_thermophysics_const_thermooptics_const_boundary_conditions_NOT_const(i, n - 1)
            ...
            For i As Integer = 0 To UBound(T, 1)
                ...
                If HT_element(i).T_SET Is Nothing Then
                    ...
                    With HT_element(i).Element
                        ...
                        T(i, n) = T(i, n - 1) + Step_time * Q(i, n - 1) / (.Medium.c * .Field.Mass)

                        If T(i, n) <= 0 Or Double.IsInfinity(T(i, n)) Or Double.IsNaN(T(i, n)) Then

                            Throw New System.Exception("In element " & HT_element(i).Element.Number.ToString & " temperature became negative on step number " & n.ToString)
    ...
    ''' <summary>
    ''' This sub solve problem using simple Euler integration method. The assumptions are: 1. Constant time step. 2. Constant thermophysics. 3. Constant thermooptics 4. Constant boundary conditions 5. Fixed radiation heat sources.
    ''' </summary>
    ''' <remarks></remarks>
    Sub Euler_const_time_step_TP_C_TO_C_BC_C_RS_C()
        ...
        For n As Integer = 1 To Lenght_time / Step_time
            ...
            For i As Integer = 0 To UBound(T, 1)
                ...
                If HT_element(i).T_SET Is Nothing Then
                    ...
                    Q(i, n - 1) = Calc_Q_thermophysics_const_thermooptics_const_boundary_conditions_const(i, n - 1)
    ...
    ''' <summary>
    ''' This sub solve problem using simple Euler integration method.
    ''' The assumptions are: 1. Constant time step. 2. Constant thermophysics. 3. Constant thermooptics
    ''' 4. Variable boundary conditions 5. Fixed radiation heat sources.
    ''' </summary>
    ''' <remarks></remarks>
    Sub Euler_const_time_step_TP_C_TO_C_BC_V_RS_C()

        ' Переменные для хранения мощностей
        Dim Q_R_income, Q_R_outcome, Q_lambda, Q_Heat_source, Q_Radio_source As Double

        ' Вычисляем количество шагов для хранения информации

        If Save_step_time = 0 Then Save_step_time = Step_time

        ' В какой следующий момент времени будем сохраняться
        Dim Next_time_to_save As Double

        ' Номер шага сохранения
        Dim Save_N_step As Integer = -1

        ' Заводим массивы для хранения температур на несколький предыдущих шагах и тепловыделений на текущем шаге

        Dim Temp(UBound(HT_element), 1), Power(UBound(HT_element)) As Double

        ' Начинаем отчет времени

        ReDim time(Lenght_time / Save_step_time)
        ...
        For i As Integer = 0 To UBound(Temp, 1)
            ...
            Temp(i, 1) = HT_element(i).T_0

        Next

        ' На каком шагу будем сохраняться

        Next_time_to_save = Start_time
        ...
        For n As Integer = 1 To Lenght_time / Step_time
            ...
            If Next_time_to_save <= Current_time Then

                Save_N_step += 1

                time(Save_N_step) = Current_time

                For i As Integer = 0 To UBound(HT_element)

                    T(i, Save_N_step) = Temp(i, 1)

                    Q(i, Save_N_step) = Power(i)

                Next i

                Next_time_to_save += Save_step_time
            ...
            For i As Integer = 0 To UBound(FEM.T_SET)
                ...
                For j As Integer = 0 To UBound(FEM.T_SET(i).Element)
                    ...
                    Temp(FEM.T_SET(i).Element(j).HT_element.Number, 1) = FEM.T_SET(i).Temp(j)
            ...
            ' рассчитываем тепловой поток на текущем шаге, проверяя не задана ли температура для этого элемента 
            For n_Element As Integer = 0 To UBound(HT_element)
                ...
                If HT_element(n_Element).T_SET Is Nothing Then

                    ' Рассчитываем тепловой поток во все стороны элемента за счет излучения

                    Q_R_outcome = Q_R_outcome_vector(n_Element) * Temp(n_Element, 1) ^ 4

                    ' Рассчитываем тепловой поток к элементу от других элементов за счет излучения

                    For i As Integer = 0 To UBound(Temp, 1)

                        Q_R_income += B_matrix_element(i, n_Element) * TOP_item.sigma * Temp(i, 1) ^ 4
                    ...
                    For i As Integer = 0 To UBound(HT_element(n_Element).Heat_conductivity)
                        ...
                        Q_lambda += HT_element(n_Element).Heat_conductivity(i).Conductivity * (Temp(HT_element(n_Element).Heat_conductivity(i).HT_Element.Number, 1) - Temp(n_Element, 1))
                    ...
                    If UBound(HT_element(n_Element).Heat_source) >= 0 Then
                        ...
                        For i As Integer = 0 To UBound(HT_element(n_Element).Heat_source)
                            ...
                            ' Тепловыделение в источнике может зависеть от температуры элемента, так что добавляем ее в аргумент
                            Argument.Coord(0) = Temp(n_Element, 1)
                    ...
                    Power(n_Element) = Q_R_income - Q_R_outcome + Q_lambda + Q_Heat_source + Q_Radio_source
            ...
            ' циклы по элементам
            ' рассчитываем температуру на текущем шаге, проверяя не задана ли уже температура для этого элемента 
            For i As Integer = 0 To UBound(HT_element, 1)

                If HT_element(i).T_SET Is Nothing Then

                    With HT_element(i).Element

                        Temp(i, 0) = Temp(i, 1) + Step_time * Power(i) / (.Medium.c * .Field.Mass)

                        If Temp(i, 0) <= 0 Or Double.IsInfinity(Temp(i, 0)) Or Double.IsNaN(Temp(i, 0)) Then
        ...
        time(UBound(time)) = Start_time + Lenght_time

        For i As Integer = 0 To UBound(HT_element)

            T(i, UBound(T, 2)) = Temp(i, 1)

            Q(i, UBound(Q, 2)) = Power(i)
    ...
    '                        Throw New System.Exception("In element " & HT_element(i).Number.ToString & " temperature became negative on step number " & n.ToString)
    ...
    Sub Euler_var_T_step_with_smoothing_MATRIXLESS(Optional ByRef N_step_start As Integer = 1)
        ...
        '' массив, в который кладем мощности всех элементов на шаге n
        'Dim Q_in(UBound(HT_element)) As Double
        ...
        '        Q_in(i) = HT_element(i).HT_Step(n).Q
        ...
        '            delta_t_min(i) = (K_T_min - 1) * HT_element(i).HT_Step(n - 1).T * .Medium.Calc_c(HT_element(i).Argument) * .Field.Mass / Q_in(i)

        '            delta_t_max(i) = (K_T_max - 1) * HT_element(i).HT_Step(n - 1).T * .Medium.Calc_c(HT_element(i).Argument) * .Field.Mass / Q_in(i)
        ...
        '                    'HT_element(i).HT_Step(n).T = HT_element(i).HT_Step(n - 1).T + Step_time * Q_in(i) / (.Medium.Calc_c(HT_element(i).Argument) * .Field.Mass)
        '                    '.Field.Temperature = HT_element(i).HT_Step(n).T

        '                    HT_element(i).HT_Step(n).T = Calc_T(HT_element(i), HT_element(i).HT_Step(n - 1).T, HT_element(i).HT_Step(n - 1).T, Q_in(i) * Step_time / .Field.Mass)
        ...
        '                    HT_element(i).HT_Step(n).T = Calc_T(HT_element(i), HT_element(i).HT_Step(n - 1).T, HT_element(i).HT_Step(n - 2).T, Q_in(i) * Step_time / .Field.Mass)
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Sub Euler_var_time_step_TP_C_TO_C_BC_V_RS_C()

        Dim Progress As Integer = 0

        Dim Old_Progress As Integer = -1

        ' Массивы для хранения возможных шагов по времени 
        ...
        Do
            ...
            For n_Element As Integer = 0 To UBound(HT_element)
                ...
                If HT_element(n_Element).T_SET Is Nothing Then
                    ...
                    For i As Integer = 0 To UBound(Temp, 1)
                        ...
                        Q_R_income += B_matrix(i, n_Element) * TOP_item.sigma * Temp(i, 1) ^ 4
                    ...
                    If Math.Abs(Power(n_Element)) > 1.0E+20 Then

                        Throw New System.Exception("In element " & HT_element(n_Element).Element.Number.ToString & " power income became extemely high on step number " & n.ToString)
            ...
            For i As Integer = 0 To UBound(HT_element)
                ...
                With HT_element(i).Element
                    ...
                    delta_t_min(i) = (K_T_min - 1) * Temp(i, 1) * .Medium.c * .Field.Mass / Power(i)

                    delta_t_max(i) = (K_T_max - 1) * Temp(i, 1) * .Medium.c * .Field.Mass / Power(i)
            ...
            If Progress + 1 <= Current_time / Lenght_time * 100 Then
        ...
        ReDim Preserve time(Save_N_step + 1)

        ReDim Preserve T(UBound(T, 1), Save_N_step + 1)

        ReDim Preserve Q(UBound(Q, 1), Save_N_step + 1)
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Sub Euler_var_time_step_TP_C_TO_C_BC_V_RS_C_TEST()
        ...
        Do
            ...
            For n_Element As Integer = 0 To UBound(HT_element)
                ...
                If HT_element(n_Element).T_SET Is Nothing Then
                    ...
                    For i As Integer = 0 To UBound(HT_element(n_Element).Heat_radiation)

                        Q_R_income += HT_element(n_Element).Heat_radiation(i).Coefficient * Temp(HT_element(n_Element).Heat_radiation(i).n_Emitter_HT_Element, 1) ^ 4
            ...
            If max_delta_t > Lenght_time / 10000 Then max_delta_t = Lenght_time / 10000

            If max_delta_t > Save_step_time Then max_delta_t = Save_step_time
    ...
    Public Sub Calc_Elements_Faces_codes()
        ...
        ReDim Color_codes(UBound(FEM.Shining_face), UBound(T, 2))
        ...
        If Colors_from_entire_solution Then
            ...
            For i As Integer = 0 To UBound(Color_codes, 1)

                For j As Integer = 0 To UBound(Color_codes, 2)

                    Color_codes(i, j) = Field_color(T(FEM.Shining_face(i).HT_element.Number, j), Min_T, Max_T)


                    Color_codes(i, j).Text = "Element " & FEM.Shining_face(i).HT_element.Element.Number.ToString & "." & Chr(13) & _
                    "Temperature = " & T(FEM.Shining_face(i).HT_element.Number, j) & " K." & Chr(13) & _
                    "Heat input power = "

                    If Q(FEM.Shining_face(i).HT_element.Number, j) > 0 Then

                        Q_text = "+" & Q(FEM.Shining_face(i).HT_element.Number, j).ToString() & "W."
        ...
        Else
            ...
            For j As Integer = 0 To UBound(T, 2)
    ...
    Public Sub Prepare_Draw_Temperature(ByRef i_Step As Integer)
        ...
        For i As Integer = 0 To UBound(Color_codes, 1)
            ...
            'Text = "Element " & FEM.Shining_face(i).HT_element.Element.Number.ToString & "." & Chr(13) & _
            '"Temperature = " & T(FEM.Shining_face(i).HT_element.Number, i_Step) & " K." & Chr(13) & _
            '"Heat input power = "

            'If Q(FEM.Shining_face(i).HT_element.Number, i_Step) > 0 Then

            '    Q_text = "+" & Q(FEM.Shining_face(i).HT_element.Number, i_Step).ToString() & "W."
            ...
            FEM.Shining_face(i).Code = Color_codes(i, i_Step)

            'FEM.Shining_face(i).Text = Text
    ...
    Public Sub Prepare_Elements_to_show_before_analysis()
        ...
        For i As Integer = 0 To UBound(HT_element)
            ...
            For j As Integer = 0 To UBound(HT_element(i).Face)
                ...
                HT_element(i).Face(j).Code.Text = Text & Text_face
    ...
    Public Sub Detect_Min_Max_T()
        ...
        For i As Integer = 0 To UBound(T, 1)
            ...
            For j As Integer = 0 To UBound(T, 2)
                ...
                If T(i, j) > Max_T Then Max_T = T(i, j)

                If T(i, j) < Min_T Then Min_T = T(i, j)
    ...
    Public Sub Detect_Min_Max_T_on_current_step()
        ...
        For i As Integer = 0 To UBound(T, 1)
            ...
            If T(i, Me.Current_step) > Max_T Then Max_T = T(i, Me.Current_step)

            If T(i, Me.Current_step) < Min_T Then Min_T = T(i, Me.Current_step)
...
Public Class HT_Element
    ...
    ''' <summary>
    ''' Specific heat capacity of elements
    ''' </summary>
    ''' <remarks></remarks>
    Public c As Double

    ''' <summary>
    ''' Initial temperature of element before analysis
    ''' </summary>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Array of element's conductivity heat transfer paths.
    ''' For example, if element has 4 neighbours, 
    ...
    ''' <summary>
    ''' Array of element's radiation heat transfer paths.
    ''' For example, if element absorbs radiation from 4 elements, 
    ''' then Heat_radiation array for this element will fill with 4 items.
    ''' </summary>
    ''' <remarks></remarks>
    Public Heat_radiation() As Heat_radiation
...
Public Class Heat_conductivity
    ...
    ''' <summary>
    ''' The number of element, that involve in heat transfering
    ''' </summary>
    ''' <remarks></remarks>
    Public n_HT_Element As Integer

    ''' <summary>
    ''' The area of this element for transfer heat to boundary 
    ''' </summary>
    ''' <remarks></remarks>
    Public Area_1 As Double

    ''' <summary>
    ''' The area of neighbouring element to boundary 
    ''' </summary>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Conductivity between connected elements, W/K
    ''' </summary>
    ''' <remarks></remarks>
    Public Conductivity As Double

End Class

''' <summary>
''' This class describes radiation heat transfer link between two elements.
''' It was done to replace B_matrix of radiation exchange factors.
''' The element, that absorbs radiation is own the Heat_radiation entity.
''' The element, that emit radiation 
''' </summary>
''' <remarks></remarks>
<Serializable()> _
Public Class Heat_radiation

    ''' <summary>
    ''' The number of element, that involve in heat transfering
    ''' </summary>
    ''' <remarks></remarks>
    Public n_Emitter_HT_Element As Integer

    ''' <summary>
    ''' Coefficient Conductivity between connected elements, W/K
    ''' </summary>
    ''' <remarks></remarks>
